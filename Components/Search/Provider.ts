...
function useSearchSection() {
  ...
  const [filter, setFilter] = useState({ keyword: id ? id.toString() : null, blockChainIdList: [], first: 0, last: 10, before: null, after: null, of: ProjectListOf.NEWEST })

  const { data, loading } = useQuery(projectListByFilter, {
    variables: { ...filter, keyword: id ? id.toString() : null },
